...
class GitHub_Deploy_Setup_Wizard {
    ...
    public function __construct(
        GitHub_Deploy_Settings $settings,
        GitHub_API $github_api,
        GitHub_Deploy_Database $database,
        GitHub_Deploy_Debug_Logger $logger,
        GitHub_Deploy_App_Connector $app_connector
    ) {
    ...
    public function should_show_wizard(): bool {
        ...
        if (get_option('github_deploy_wizard_skipped')) {
            return false;
        }
    ...
    public function can_resume_wizard(): bool {
        ...
        for ($i = 1; $i <= 6; $i++) {
            if (get_transient('github_deploy_wizard_step_' . $i)) {
                return true;
            }
        }
        return false;
    ...
    public function add_wizard_page(): void {
        add_submenu_page(
            ...
            __('Setup Wizard', 'github-auto-deploy'),
            __('Setup Wizard', 'github-auto-deploy'),
            'manage_options',
            'github-deploy-wizard',
            [$this, 'render_wizard']
        );
    }
    ...
    public function maybe_redirect_to_wizard(): void {
        ...
        if (get_transient('github_deploy_activation_redirect')) {
            delete_transient('github_deploy_activation_redirect');
    ...
    public function enqueue_assets($hook): void {
        ...
        if ($hook !== 'admin_page_github-deploy-wizard') {
            return;
        }
        ...
        wp_enqueue_style(
            'github-deploy-wizard',
            GITHUB_DEPLOY_PLUGIN_URL . 'admin/css/setup-wizard.css',
            [],
        ...
        wp_enqueue_script(
            'github-deploy-wizard',
            GITHUB_DEPLOY_PLUGIN_URL . 'admin/js/setup-wizard.js',
            ['jquery', 'select2'],
            GITHUB_DEPLOY_VERSION,
            true
        );
        ...
        // Localize script
        wp_localize_script('github-deploy-wizard', 'githubDeployWizard', [
            'ajaxUrl' => admin_url('admin-ajax.php'),
            'nonce' => wp_create_nonce('github_deploy_wizard'),
            'currentStep' => isset($_GET['step']) ? intval($_GET['step']) : 1,
            'isConnected' => $this->app_connector->is_connected(),
            'dashboardUrl' => admin_url('admin.php?page=github-deploy'),
            'strings' => [
                'error' => __('An error occurred. Please try again.', 'github-auto-deploy'),
                'loading' => __('Loading...', 'github-auto-deploy'),
                'skipConfirm' => __('Are you sure you want to skip setup? You can configure settings manually later.', 'github-auto-deploy'),
                'exitConfirm' => __('Exit setup wizard? Your progress will be saved.', 'github-auto-deploy'),
            ]
        ]);
    }
    ...
    public function render_wizard(): void {
        ...
        include GITHUB_DEPLOY_PLUGIN_DIR . 'templates/setup-wizard/wizard-container.php';
    }
    ...
    public function ajax_get_repos(): void {
        ...
        if (!$repos['success']) {
            ...
            wp_send_json_error(['message' => $repos['message'] ?? __('Failed to fetch repositories', 'github-auto-deploy')]);
        }
    ...
    public function ajax_get_branches(): void {
        ...
        if (empty($repo_full_name)) {
            wp_send_json_error(['message' => __('Repository name required', 'github-auto-deploy')]);
        }
        ...
        if (!$branches['success']) {
            ...
            wp_send_json_error(['message' => $branches['message'] ?? __('Failed to fetch branches', 'github-auto-deploy')]);
        }
    ...
    public function ajax_get_workflows(): void {
        ...
        if (empty($repo_owner) || empty($repo_name)) {
            wp_send_json_error(['message' => __('Repository information required', 'github-auto-deploy')]);
        }
    ...
    public function ajax_validate_repo(): void {
        ...
        if (!$result['success']) {
            wp_send_json_error(['message' => __('Cannot access repository', 'github-auto-deploy')]);
        }
    ...
    public function ajax_save_step(): void {
        ...
        if ($step < 1 || $step > 6) {
            wp_send_json_error(['message' => __('Invalid step', 'github-auto-deploy')]);
        }
    ...
    public function ajax_complete(): void {
        ...
        update_option('github_deploy_wizard_completed_at', current_time('mysql'));
        ...
        wp_send_json_success([
            'redirect' => admin_url('admin.php?page=github-deploy&wizard_completed=1')
        ]);
    }
    ...
    public function ajax_skip(): void {
        ...
        update_option('github_deploy_wizard_skipped', true);

        $this->logger->log('Setup_Wizard', 'Wizard skipped by user');

        wp_send_json_success([
            'redirect' => admin_url('admin.php?page=github-deploy-settings&wizard_skipped=1')
        ]);
    }
