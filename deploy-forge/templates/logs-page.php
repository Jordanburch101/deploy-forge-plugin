 ...
 *
 * Displays the debug log viewer with controls for refreshing,
 * clearing, and configuring log output.
 *
	...
	(function($) {
		...
		const LogsViewer = {
				...
				$.ajax({
					...
					success: function(response) {
						if (response.success) {
							...
							alert('<?php echo esc_js( __( 'Failed to load logs:', 'deploy-forge' ) ); ?> ' + (response.data.message || '<?php echo esc_js( __( 'Unknown error', 'deploy-forge' ) ); ?>'));
						}
					},
					error: function() {
						alert('<?php echo esc_js( __( 'Failed to load logs. Please try again.', 'deploy-forge' ) ); ?>');
					},
					complete: function() {
			...
			clearLogs: function(e) {
				...
				if (!confirm('<?php echo esc_js( __( 'Are you sure you want to clear all debug logs? This cannot be undone.', 'deploy-forge' ) ); ?>')) {
					return;
				}
				...
				$.ajax({
					...
					success: function(response) {
						if (response.success) {
							$output.text('<?php echo esc_js( __( 'Logs cleared. New entries will appear here.', 'deploy-forge' ) ); ?>');
							$('#log-size strong').text('0 bytes');
						} else {
							alert('<?php echo esc_js( __( 'Failed to clear logs:', 'deploy-forge' ) ); ?> ' + (response.data.message || '<?php echo esc_js( __( 'Unknown error', 'deploy-forge' ) ); ?>'));
						}
					},
					error: function() {
						alert('<?php echo esc_js( __( 'Failed to clear logs. Please try again.', 'deploy-forge' ) ); ?>');
					},
					complete: function() {
						$spinner.removeClass('is-active');
					}
