...
class GitHub_Deploy_App_Connector {
    ...
    public function handle_oauth_callback(): void {
        ...
        $installation_id = intval($_GET['installation_id'] ?? 0);
        ...
        if (empty($api_key) || empty($installation_id)) {
            $this->logger->error('GitHub_App_Connector', 'Missing API key or installation ID in callback');
            add_action('admin_notices', function() {
                echo '<div class="notice notice-error"><p>' . 
                    esc_html__('Failed to connect to GitHub: Missing connection data.', 'github-auto-deploy') . 
                    '</p></div>';
            });
            return;
        }
