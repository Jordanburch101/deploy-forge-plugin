...
1. **Download the plugin**
   ...
   git clone <repository-url> deploy-forge
   ```

2. **Activate the plugin**
   - Go to WordPress admin → Plugins
   - Activate "Deploy Forge"
...
2. Click "Add webhook"
3. **Payload URL**: Copy from WordPress Admin → Deploy Forge → Settings
4. **Content type**: `application/json`
5. **Secret**: Copy from WordPress Admin → Deploy Forge → Settings
...
1. Go to **Deploy Forge → Dashboard**
2. Click "Deploy Now"
...
**Actions:**
- `deploy_forge_completed` - Fires after successful deployment
- `deploy_forge_rolled_back` - Fires after rollback
- `deploy_forge_webhook_received` - Fires when webhook received
- `deploy_forge_rate_limit_warning` - Fires when rate limit is low
