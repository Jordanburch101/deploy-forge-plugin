...
(function ($) {
   ...
   */
  DeployForgeAjax.request = function (options) {
    const ajaxUrl = window.deployForgeAdmin?.ajaxUrl || window.deployForgeWizard?.ajaxUrl || '/wp-admin/admin-ajax.php';
   ...
   * @returns {jqXHR} jQuery AJAX object
   */
  DeployForgeAjax.loadRepositories = function (selectElement, action, nonce, onSuccess, onError) {
   ...
   * @returns {jqXHR} jQuery AJAX object
   */
  DeployForgeAjax.loadWorkflows = function (owner, repo, selectElement, action, nonce, onSuccess, onError) {
    // Clear existing options
   ...
   * @returns {jqXHR} jQuery AJAX object
   */
  DeployForgeAjax.loadBranches = function (selectElement, action, nonce, onSuccess, onError) {
    // Clear existing options
    ...
    return DeployForgeAjax.request({
      ...
      onSuccess: function (data) {
        ...
        branches.forEach(function (branch) {
          const branchName = typeof branch === 'string' ? branch : (branch.name || branch.label);
          const option = $('<option></option>')
            .val(branchName)
            .text(branchName);
   ...
   */
  DeployForgeAjax.initSelect2 = function (element, placeholder, customOptions) {
   ...
   */
  DeployForgeAjax.showError = function (message, container) {
   ...
   * @param {jQuery} container Container to show message in (optional)
   */
  DeployForgeAjax.showSuccess = function (message, container) {
   ...
   */
  DeployForgeAjax.clearMessage = function (container) {
    if (container && container.length) {
